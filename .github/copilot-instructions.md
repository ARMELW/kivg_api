...
Le projet utilise Docker et est configuré pour le déploiement sur des plateformes cloud. Voir `Dockerfile` et `deploy.yml` pour la configuration complète.
...
```typescript
// Service de fichier de jeu
const gameFileService = new GameFileService()
const result = await gameFileService.uploadGameFile(file) // { id: string, url: string }

// Service de couverture de jeu
const gameCoverService = new GameCoverService()
const coverResult = await gameCoverService.uploadGameCover(imageFile)

// Use case complet
const createGameUseCase = new CreateGameUseCase(gameRepository)
const game = await createGameUseCase.execute({
  title: 'Mon Jeu',
  file: '/uploads/games/abc123.zip',
  coverUrl: '/uploads/game-covers/def456.jpg',
  lessonId: 'lesson-id'
})
```

La suppression d'un jeu nettoie automatiquement tous les fichiers associés (fichier de jeu et couverture).
